...
async function lemmatizeText(text) {
  return new Promise((resolve, reject) => {
    const python = spawn('python', [path.join(__dirname, '..', '..', 'python', 'lemmatize.py'), text]); // Путь к скрипту!
        let data = '';
        let error = '';
        ...
        python.on('close', (code) => {
            if (code !== 0) {
                console.error(`Ошибка от Python скрипта, код: ${code}, сообщение: ${error}`);
...
async function search(userQuerry) {
    try {
        ...
        const { tfidf, queryVector } = calculateTfIdf(documents, processedQuery.join(" "));
        console.log(queryVector);
        // Сравниваем запрос с каждым документом
        const results = keywordsTexts.map((keyword, index) => {
            let documentVector = {};
        ...
        results.forEach(keyword => {
            console.log(`- ${keyword.id_media} с соответствием ${keyword.score * 100}%`);
        });
