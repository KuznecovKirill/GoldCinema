...
const FavoritesItem = ({favorite, onRemoved}) => {
    ...
    const onRemove = async() => {
        ...
        const {response, err} = await favoriteModule.remove({id_favorite: fav.id_favorite});
        setOnRequest(true);
...
const FavoritesPage = () => {
      ...
      const getFavorites = async () => {
        ...
        //   setFavorites([...response]);
          ...
          for (const fav of response) {
            const { response: mediaInfo, err: mediaErr } = await mediaModule.getInfo({
    ...
    const onLoadMore = () => {
      const newItems = favorites.slice(
        (page + 1) * skip,
        (page + 1) * skip + skip
      );
    ...
    const onRemoved = (id_media) => {
      ...
      setFiltredFavorites(newFavorites.slice(0, page * skip));
      ...
      <Box sx={{ ...UI.style.mainContent }}>
        <Container header={`Ваш список избранного (${count})`}>
          <Grid container spacing={1} sx={{ marginRight: "-8px!important" }}>
            {filtredFavorites.map((fav, index) => (
